...
// Simple string hash function for deterministic pricing
...
/**
 * Category-appropriate realistic Indian retail price generator.
 * Uses exact category ranges mandated by store guidelines.
 */
export function generateDemoPriceAndMRP(categoryName: string, subcategoryName: string, productType: string) {
  ...
  // End price with realistic retail numbers (990, 490, 999, 499)
  let sellingPrice = Math.round(rawPrice / 100) * 100 - 10;
  if (sellingPrice < minPrice) sellingPrice = minPrice;

  // Realistic discount between 10% and 25% (MRP > sellingPrice)
...
/**
 * Searches the Excel workbook for the authoritative catalog table,
 * rejects waste/non-catalog data, and seeds the database idempotently.
 */
export async function importCatalogFromExcel(filePath: string): Promise<CatalogImportReport> {
  ...
  try {
    ...
    // Target headers to identify the authoritative table
    ...
    // 1. Search all sheets to find the authoritative catalog table
    for (const sheetName of workbook.SheetNames) {
      ...
      for (let r = 0; r < data.length; r++) {
        ...
        if (hasAllRequired) {
          ...
          // Count rows before header in this sheet as non-catalog content
          report.ignoredNonCatalogContent += r;
          break;
        } else {
          // Non-matching row
          if (r === 0 && data.length > 0) {
            // Whole sheet might be non-catalog
          }
        }
      }

      if (!targetSheetName) {
        // Entire sheet is non-catalog content
        report.ignoredNonCatalogContent += data.length;
      } else {
        // If we found the target sheet, accumulate non-catalog row counts from remaining sheets
    ...
    // Helper to map category/dept to canonical Primary Categories
    ...
    for (let r = headerRowIndex + 1; r < rawRows.length; r++) {
      ...
      // 5. Generate Deterministic Sellable Product
      ...
      // Deterministic price & stock calculation
    ...
    // Format mandatory exact report output string
    report.reportText = `
