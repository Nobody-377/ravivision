...
async function main() {
  ...
  // 1. Create Default Admin Account with secure password procedures
  ...
  if (!existingAdmin) {
    ...
    await prisma.adminUser.create({
      data: {
        ...
        passwordHash,
        name: 'Store Owner',
        mustChangePassword: !envPassword, // Enforce password change if randomly generated
      },
    });
    ...
    if (!envPassword) {
      console.log('NOTICE: You will be prompted to change this password on first login.');
    }
    console.log('======================================================\n');
  } else {
  ...
  // 2. Configure Allowed Delivery Zones (Kargahar Region & Surrounding Tehsils)
  const allowedZones = [
  ...
  // Remove any pincodes not in the exclusive allowed list
  await prisma.deliveryZone.deleteMany({
    where: {
      pincode: { notIn: allowedPincodes }
    }
  });

  // Upsert the 10 Kargahar region pincodes as active delivery zones
